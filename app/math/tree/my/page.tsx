...
const MATH_SLUGS = new Set(
  MATH_CATEGORIES.flatMap((cat) => cat.topics.map((t) => slugify(t)))
);

export default function MathMyTreePage() {
  ...
  // Filter to math topics only
  const mathHistory = useMemo(
    () => topicHistory.filter((t) => MATH_SLUGS.has(t.slug)),
    [topicHistory]
  );

  const fetchTree = useCallback(async (history: TopicHistoryItem[]) => {
    const slugs = history.map((t) => t.slug);
    const cacheKey = computeTreeCacheKey(slugs, "tmi10_math_tree_");
    ...
    try {
      ...
      const res = await fetch("/api/topics/tree", {
        ...
        body: JSON.stringify({ topics: topicNames, mode: "math" }),
      });

      if (!res.ok) throw new Error("Failed to generate tree");

      const data: LearningTreeData = await res.json();
      setCachedTree(computeTreeCacheKey(slugs, "tmi10_math_tree_"), data);
      setTreeData(data);
    } catch (e) {
  ...
  useEffect(() => {
    dataLayer.getTopicHistory().then((history) => {
      ...
      const math = history.filter((t) => MATH_SLUGS.has(t.slug));
      if (math.length >= 3) {
        fetchTree(math);
      } else {
        setLoading(false);
    ...
    <main className="min-h-screen flex flex-col items-center px-4 py-16 relative overflow-hidden">
      ...
      >
        ...
        <p className="text-white/30 text-sm font-serif max-w-md mx-auto">
          Your personal math journey — see how your explored topics connect.
        </p>
      </motion.div>
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-white/30 text-sm font-sans">
            Mapping your math connections...
          </p>
        </motion.div>
      ) : mathHistory.length < 3 ? (
        <motion.div
          className="text-center py-20"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
          <p className="text-white/40 text-sm font-sans mb-2">
            Explore at least 3 math topics to build your learning tree.
          </p>
          <p className="text-white/25 text-xs font-sans">
            You&apos;ve explored {mathHistory.length} math topic{mathHistory.length !== 1 ? "s" : ""} so far.
          </p>
          <button
            onClick={() => router.push("/math")}
            className="mt-6 px-5 py-2.5 rounded-xl bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 text-sm font-sans hover:bg-indigo-500/20 transition-colors"
          >
            Explore Math Topics
          </button>
        </motion.div>
